...
public class AccountTransactionController : ControllerBase
{
    ...
    public async Task<ActionResult> Get()
    {
        try
        {
            var _list = await dbContext.Set<AccountTransaction>().ToListAsync();
            if (_list == null)
            {
    ...
    public async Task<ActionResult> Get(int id)
    {
        try
        {
            var item = await dbContext.Set<AccountTransaction>().Where(x => x.Id == id).FirstOrDefaultAsync();
            if (item == null)
            {
                return NotFound(); //404 Not Found : Ýstenen kaynak bulunamadý.
            }
    ...
    [HttpPost]
    public async Task<ActionResult> Post([FromBody] AccountTransaction item)
    {
        try
        {
            if (item == null)
    ...
    [HttpPut("{id}")]
    public async Task<ActionResult> Put(int id, [FromBody] AccountTransaction item)
    {
        try
        {
            if (id != item.Id)
            ...
            dbContext.AccountTransactions.Update(_item);
            await dbContext.SaveChangesAsync();
